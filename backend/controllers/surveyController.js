...
const getAvailableSurveys = async (req, res) => {
  try {
    ...
    for (const schedule of schedules) {
      ...
      if (
        schedule.targetEmployeeIds &&
        typeof schedule.targetEmployeeIds === "string" &&
        schedule.targetEmployeeIds.trim() !== ""
      ) {
        try {
          targetEmployeeIds = JSON.parse(schedule.targetEmployeeIds);
        } catch {
          targetEmployeeIds = [];
        }
      ...
      const matchesDepartment =
        Array.isArray(targetAudience?.departments) &&
        targetAudience.departments.includes(currentUser.department);
      const matchesProgram =
        Array.isArray(targetAudience?.programs) &&
        targetAudience.programs.includes(currentUser.programType);
      const matchesEmployee =
        Array.isArray(targetEmployeeIds) &&
        targetEmployeeIds.includes(currentUser.id);
    ...
    const surveys = await Survey.findAll({
      where: {
        ...
        status: "active",
        [Op.or]: [{ dueDate: { [Op.gt]: currentDate } }, { dueDate: null }],
      },
      include: [
        {
    ...
    const completedSurveyIds = new Set(
      completedResponses.map((r) => r.surveyId)
    );
    ...
    const formattedSurveys = allSurveys.map((survey) => {
      ...
      const hasCompleted =
        ...
        surveyJson.responses.some((r) => r.status === "completed");
      return {
        ...surveyJson,
...
const getSurveyById = async (req, res) => {
  try {
    ...
    if (
      survey.responses &&
      survey.responses.some((r) => r.status === "completed")
    ) {
...
const getTeamSurveyResults = async (req, res) => {
  try {
    ...
    if (!["supervisor", "manager", "hr"].includes(req.user.role)) {
      return res
        .status(403)
        .json({ message: "Not authorized to view team results" });
    }
    ...
    switch (view) {
      case "aggregated":
        ...
        result = responses.reduce((acc, response) => {
          ...
          acc[surveyId].participationRate =
            (acc[surveyId].totalResponses / teamMembers.length) * 100;
          ...
          response.questionResponses.forEach((qr) => {
            if (qr.question.type === "rating" && qr.ratingValue) {
              ...
              acc[surveyId].averageScores[qr.question.id].totalScore +=
                qr.ratingValue;
...
const getTeamSurveyCompletionStatus = async (req, res) => {
  try {
    ...
    if (!validStatuses.includes(status)) {
      return res.status(400).json({
        message:
          "Invalid status. Must be one of: draft, active, completed, all",
      });
    }
    ...
    if (!["supervisor", "manager", "hr"].includes(req.user.role)) {
      return res
        .status(403)
        .json({ message: "Not authorized to view team completion status" });
    }
    ...
    const teamCompletionStatus = teamMembers.map((member) => {
      const memberResponses = surveyResponses.filter(
      ...
      const surveyStatus = surveys.map((survey) => {
        ...
        return {
          ...
          isOverdue:
            survey.dueDate &&
            new Date() > new Date(survey.dueDate) &&
...
const getSurveysWithResponses = async (
  surveyType,
  userIds,
  startDate,
  endDate
...
const calculateMainAnalytics = (surveys, employees) => {
  ...
  const participationRate =
    employees.length > 0 ? (uniqueRespondents / employees.length) * 100 : 0;

  const ratingResponses = surveys.flatMap((s) =>
    s.responses.flatMap((r) =>
      r.questionResponses.filter(
        (qr) => qr.question.type === "rating" && qr.ratingValue
      )
    )
  );
  const averageSatisfaction =
    ratingResponses.length > 0
      ? ratingResponses.reduce((sum, qr) => sum + qr.ratingValue, 0) /
  ...
  return {
    ...
    participationRate,
    averageSatisfaction,
    byType: surveys.reduce(
      (acc, survey) => ({ ...acc, [survey.type]: (acc[survey.type] || 0) + 1 }),
      {}
    ),
  };
};
...
const calculateSupervisorPerformance = (
  supervisors,
  surveys,
  allDepartmentEmployees
) => {
  return supervisors.map((supervisor) => {
    const supervisorTeamEmployeeIds = allDepartmentEmployees
      .filter((e) => e.supervisorId === supervisor.id)
      .map((e) => e.id);
    const teamResponses = surveys.flatMap((s) =>
      s.responses.filter((r) => supervisorTeamEmployeeIds.includes(r.userId))
    );
    ...
    return {
      ...
      completionRate:
        supervisorTeamEmployeeIds.length > 0 && surveys.length > 0
          ? (teamResponses.length /
              (surveys.length * supervisorTeamEmployeeIds.length)) *
            100
          : 0,
...
const getDepartmentAnalytics = async (req, res) => {
  try {
    if (!["manager", "hr"].includes(req.user.role)) {
      return res
        .status(403)
        .json({ message: "Not authorized to view department analytics" });
    }
    let {
    ...
    if (
      ...
      return res
        .status(403)
        .json({
          message: "Managers can only view their own department analytics",
        });
    }
    const { startDate, endDate } = calculateDateRange(timeframe);
    // 1. Get Department Employees
    const employees = await getDepartmentUsers(department, programType, [
      "employee",
    ]);
    const employeeIds = employees.map((e) => e.id);
    console.log(
    ...
    for (const schedule of schedules) {
      ...
      const matchesDepartment =
        department &&
        Array.isArray(targetAudience?.departments) &&
        targetAudience.departments.includes(department);
      const matchesProgram =
        programType &&
        Array.isArray(targetAudience?.programs) &&
        targetAudience.programs.includes(programType);
      const matchesEmployee =
        Array.isArray(targetEmployeeIds) &&
        employeeIds.some((id) => targetEmployeeIds.includes(id));
      if (matchesDepartment || matchesProgram || matchesEmployee) {
    ...
    const totalCompletedEmployees = Array.from(completedEmployeeIds).filter(
      (id) => assignedEmployeeIds.has(id)
    ).length;
    const participationRate =
      totalAssignedEmployees > 0
        ? Math.min(
            (totalCompletedEmployees / totalAssignedEmployees) * 100,
            100
          )
        : 0;
    const analytics = {
...
const compareWithPreviousTimeframe = async (
  department,
  startDate,
  endDate,
  employeeIds
...
const identifyEmergingPatterns = (surveys) => {
  ...
  const sortedSurveys = [...surveys].sort(
    (a, b) =>
      new Date(a.responses[0]?.submittedAt) -
      new Date(b.responses[0]?.submittedAt)
  );
  ...
  if (sortedSurveys.length >= 2) {
    ...
    if (Math.abs(lastAvgRating - firstAvgRating) >= 0.5) {
      patterns.ratingTrends.push({
        ...
        description: `Overall satisfaction has ${
          lastAvgRating > firstAvgRating ? "improved" : "declined"
        } by ${Math.abs(lastAvgRating - firstAvgRating).toFixed(1)} points`,
      });
    }
  }
...
const generateRecommendations = (surveys) => {
  ...
  const participationRate = calculateParticipationRate(
    surveys,
    surveys.reduce((max, s) => Math.max(max, s.responses.length), 0)
  );
  ...
  if (participationRate < 70) {
    recommendations.push({
      ...
      suggestion:
        "Implement strategies to increase survey participation rate, such as regular reminders and highlighting the importance of feedback.",
    });
  }
...
const calculateEngagementMetrics = (surveys, employees) => {
  ...
  surveys.forEach((survey) => {
    survey.responses.forEach((response) => {
      const responseTime =
        new Date(response.submittedAt) - new Date(survey.createdAt);
      ...
      if (hoursToRespond <= 24) {
        metrics.overall.responseTimeAnalysis.responseTimeDistribution
          .within24h++;
      } else if (hoursToRespond <= 48) {
        metrics.overall.responseTimeAnalysis.responseTimeDistribution
          .within48h++;
      } else if (hoursToRespond <= 168) {
        // 1 week
        metrics.overall.responseTimeAnalysis.responseTimeDistribution
          .within1week++;
      } else {
...
const monitorSurveyParticipation = async (req, res) => {
  try {
    ...
    const monitoringData = surveys.map((survey) => {
      ...
      const uniqueRespondents = new Set(survey.responses.map((r) => r.userId))
        .size;
      ...
      const filteredResponses = survey.responses.filter((response) => {
        ...
        if (programType && response.user.programType !== programType)
          return false;
        return true;
      });
        ...
        if (schedule.targetAudience) {
          try {
            ...
            parsedTargetAudience =
              typeof schedule.targetAudience === "string"
                ? JSON.parse(schedule.targetAudience)
                : schedule.targetAudience;
          } catch (e) {
            console.warn(
              `Failed to parse targetAudience for schedule ${schedule.id}:`,
              e
            );
        ...
        metrics: {
          ...
          completionRate:
            filteredResponses.length > 0
              ? (filteredResponses.length / totalResponses) * 100
              : 0,
          averageResponseTime: calculateAverageResponseTime(filteredResponses),
          responseDistribution:
            calculateMonitoringResponseDistribution(filteredResponses),
        },
        schedules,
        lastResponse:
          filteredResponses.length > 0
            ? new Date(
                Math.max(
                  ...filteredResponses.map((r) => new Date(r.submittedAt))
                )
              )
            : null,
      };
  ...
  try {
    ...
    surveys.forEach((survey) => {
      survey.responses.forEach((response) => {
        ...
        Array.from(allQuestions).forEach((question) => {
          const answer = response.questionResponses.find(
            (qr) => qr.question.question === question
          );
          row.push(
            answer
              ? answer.answer ||
    ...
    switch (format.toLowerCase()) {
      case "csv": {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=survey_results.csv"
        );
        ...
        try {
          ...
          surveys.forEach((survey) => {
            survey.responses.forEach((response) => {
              ...
              // Add answers for each question
              Array.from(allQuestions).forEach((question) => {
                const questionResponse = response.questionResponses.find(
                  (qr) => qr.question.question === question
                );
                ...
                if (questionResponse) {
                  ...
                  else if (questionResponse.ratingValue)
                    answer = questionResponse.ratingValue.toString();
                  else if (questionResponse.selectedOption)
                    answer = questionResponse.selectedOption;
                }
                row.push(answer);
              });
          ...
          res.setHeader(
            "Content-Disposition",
            "attachment; filename=survey_results.xlsx"
          );
        ...
        try {
          ...
          if (req.query.surveyId) {
            ...
            if (!surveyToExport) {
              ...
              res.setHeader(
                "Content-Disposition",
                "attachment; filename=survey_results.pdf"
              );
              const doc = new PDFDocument({ margin: 50 });
              doc.pipe(res);
              doc
                .fontSize(20)
                .text("Survey Results Report", { align: "center" });
              doc.moveDown(2);
              doc
                .fontSize(14)
                .text("No survey data available for the selected survey.", {
                  align: "center",
                });
              doc.end();
              return;
          ...
          if (!surveysToExport.length) {
            doc
              .fontSize(14)
              .text("No survey data available.", { align: "center" });
            doc.end();
            return;
          }
          ...
          surveysToExport.forEach((survey, index) => {
            ...
            doc
              .fontSize(16)
              .text(survey.title || "Untitled Survey", { underline: true });
            doc
              .fontSize(12)
            ...
            const completionRate =
              survey.responses.length > 0 && survey.SurveyQuestions.length > 0
                ? (
                    (survey.responses.length / survey.SurveyQuestions.length) *
                    100
                  ).toFixed(1) + "%"
                : "N/A";
            ...
            if (
              ...
              // Table headers
              const questionHeaders = survey.SurveyQuestions.map(
                (q) => q.question
              );
              ...
              survey.responses.forEach((response) => {
                ...
                const rowData = [
                  ...
                  ...survey.SurveyQuestions.map((q) => {
                    const qr = response.questionResponses.find(
                      (qr) => qr.question.question === q.question
                    );
                    if (!qr) return "";
      ...
      default: {
        ...
        const exportData = surveys.map((survey) => {
          const responses = survey.responses.filter((response) => {
            if (!response.user) return false;
            if (department && response.user.department !== department)
              return false;
            if (programType && response.user.programType !== programType)
              return false;
            return true;
...
const updateSurveyTemplate = async (req, res) => {
  ...
  try {
    ...
    for (const questionData of questions) {
      const { id, question, type, required, options, questionOrder } =
        questionData;
    ...
    await settings.update(
      {
        defaultAnonymous:
          defaultAnonymous !== undefined
            ? defaultAnonymous
            : settings.defaultAnonymous,
        allowComments:
          allowComments !== undefined ? allowComments : settings.allowComments,
        requireEvidence:
          requireEvidence !== undefined
            ? requireEvidence
            : settings.requireEvidence,
        autoReminders:
          autoReminders !== undefined ? autoReminders : settings.autoReminders,
...
const deleteSurveyTemplateQuestion = async (req, res) => {
  try {
    ...
    const question = await SurveyQuestion.findOne({
      where: { id: questionId, surveyId: templateId },
    });
    if (!question) {
      return res.status(404).json({ message: "Question not found" });
    }
...
const getDepartmentInsights = async (req, res) => {
  try {
    if (!["manager", "hr"].includes(req.user.role)) {
      return res
        .status(403)
        .json({ message: "Not authorized to view department insights" });
    }
    const department = req.user.department;
    if (!department) {
      return res
        .status(400)
        .json({ message: "No department assigned to user" });
    }
    ...
    if (req.user.role === "manager" && req.user.department !== department) {
      return res
        .status(403)
        .json({
          message: "Managers can only view their own department insights",
        });
    }
    ...
    const surveys = await SurveyResponse.findAll({
      ...
      include: [
        { model: Survey, as: "survey" },
        {
          model: SurveyQuestionResponse,
          as: "questionResponses",
          include: [{ model: SurveyQuestion, as: "question" }],
        },
      ],
    });
    // Calculate insights
