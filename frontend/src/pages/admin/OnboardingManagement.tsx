...
const OnboardingManagement: React.FC = () => {
  ...
  const fetchEmployees = async () => {
    try {
      setLoading(true);
      const data: OnboardingProgressResponse[] =
        await onboardingService.getAllProgresses();
      // Map backend data to table row shape
      const mapped: EmployeeOnboarding[] = data.map(
        (item: OnboardingProgressResponse) => ({
          userId: item.User?.id || "",
          name: item.User?.name || "",
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    const matchesCompletion =
      emp.completionPercentage >= filters.completionRange[0] &&
  ...
  const handleEditSave = async () => {
    ...
    setActionLoading(true);
    try {
      await onboardingService.updateUserProgress(selectedEmployee.userId, {
        stage: editPhase,
        progress: editCompletion,
      });
  ...
  const fetchCurrentPhases = async (users: EmployeeOnboarding[]) => {
    ...
    await Promise.all(
      users.map(async (emp) => {
        try {
          const journey: OnboardingJourney = await onboardingService.getJourney(
            emp.userId
          );
          ...
          if (phaseTitle)
            phaseTitle =
              phaseTitle.charAt(0).toUpperCase() + phaseTitle.slice(1);
  ...
  const columns: ColumnsType<EmployeeOnboarding> = [
    {
      ...
      dataIndex: "name",
      key: "name",
      sorter: (a: EmployeeOnboarding, b: EmployeeOnboarding) =>
    ...
    },
    {
      ...
      onFilter: (value: boolean | React.Key, record: EmployeeOnboarding) =>
        record.role === value.toString(),
    },
    {
      title: "Department",
    ...
    },
    {
      ...
      render: (_: unknown, record: EmployeeOnboarding) => {
        const phase = currentPhases[record.userId] || record.currentPhase;
      ...
      render: (_: unknown, record: EmployeeOnboarding) => {
        const status =
          record.completionPercentage === 100
            ? "Completed"
            : currentStatuses[record.userId] || record.status;
      ...
      <div className="p-6">
        ...
        {alertEmployees.length > 0 && (
          <Card
            title={
              <>
                <WarningOutlined className="text-red-500 mr-2" /> Attention
                Required
              </>
            }
            className="mb-6"
          ...
          >
            <p className="mb-4">
              The following employees have delayed or stalled onboarding
              progress:
            </p>
              ...
              {alertEmployees.map((emp) => (
                <li key={emp.userId} className="mb-2">
                  ...
                  <span className="text-red-500">
                    {emp.status?.toLowerCase() === "delayed"
                      ? "Has overdue tasks"
                      : `No activity for ${emp.daysSinceLastActivity} days`}
                  </span>
                  <Button
                    type="link"
                    size="small"
        ...
        <div className="mb-4 flex gap-4">
          ...
          <Button onClick={fetchEmployees} icon={<ReloadOutlined />}>
            Refresh Table
          </Button>
        </div>
        ...
        >
          <p>
            Reset {selectedEmployee?.name}'s onboarding journey? This cannot be
            undone.
          </p>
        </Modal>
        <Modal
