...
const OnboardingDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (Object.keys(tasksByPhase).length > 0) {
      const initialCompletedTasks: Record<string, boolean> = Object.values(
        tasksByPhase
      )
        .flat()
  ...
  const fetchOnboardingDetails = async () => {
    try {
      ...
      Object.keys(tasksByPhase).forEach((phase) => {
        tasksByPhase[phase] = tasksByPhase[phase].map(
  ...
  const handleValidateTask = async (taskId: string, isValidated: boolean) => {
    try {
      ...
      message.success(
        isValidated ? "Task validated successfully" : "Task validation rejected"
      );
  ...
  const handleAdvancePhase = async () => {
    Modal.confirm({
      ...
      content:
        "Are you sure you want to advance this employee to the next phase? This action cannot be undone.",
      okText: "Yes, Advance",
  ...
  const saveProgress = async (shouldNavigate = false) => {
    try {
      ...
      const tasksToUpdate = Object.entries(completedTasks).map(
        ([taskId, isCompleted]) => ({
          id: taskId,
          completed: isCompleted,
          hrValidated:
            tasksByPhase[Object.keys(tasksByPhase)[0]].find(
              (t) => t.id === taskId
            )?.hrValidated || false,
        })
      );
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-medium mb-2">
              {employeeName}'s Onboarding Journey
            </h1>
