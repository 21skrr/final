...
import { Upload, Trash2, Eye, Users, FileText, Plus, X, CheckCircle, Search } from 'lucide-react';
...
const SupervisorResourceView: React.FC = () => {
  const [tab, setTab] = useState<'library'|'assign'|'upload'>('library');
  ...
  const [isPublic, setIsPublic] = useState(false); // team-wide default
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [res, teamData] = await Promise.all([
        resourceService.getTeamResources(),
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setTitle(''); setDescription(''); setCategory('other'); setIsPublic(false); setFile(null);
      loadData();
      setTab('library');
  ...
  const handleAssign = async () => {
    if (!assignResourceId || assignEmployeeIds.length === 0) { toast.error('Select a document and at least one employee.'); return; }
    setAssigning(true);
    try {
      await resourceService.assignResources({ resourceIds: [assignResourceId], employeeIds: assignEmployeeIds });
      toast.success('Document assigned!');
      setAssignResourceId(''); setAssignEmployeeIds([]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="glass-panel p-6 mb-6 flex flex-col md:flex-row items-start md:items-center gap-4 relative overflow-hidden bg-gradient-to-r from-blue-50/50 to-indigo-50/50 dark:from-blue-900/10 dark:to-indigo-900/10">
        ...
        <div>
          <h2 className="text-2xl font-display font-bold text-slate-900 dark:text-white mb-1">Team Document Library</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Upload documents for your team or assign to specific employees.</p>
        </div>
        <div className="md:ml-auto mt-4 md:mt-0 text-center bg-white/60 dark:bg-slate-800/40 px-6 py-3 rounded-xl border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm">
          ...
          <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-0.5">Docs</div>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200 dark:border-slate-700 pb-2">
        ...
        <button onClick={()=>setTab('assign')} className={`flex items-center gap-2 px-4 py-2 rounded-t-lg font-semibold text-sm transition-colors border-b-2 ${tab === 'assign' ? 'text-pmi-800 border-pmi-800 dark:text-pmi-400 dark:border-pmi-400 bg-pmi-50/50 dark:bg-pmi-900/10' : 'text-slate-500 border-transparent hover:text-slate-700 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-800/50 dark:hover:text-slate-200'}`}>
        ...
        <div className="animate-in fade-in duration-300">
          ...
          filtered.length === 0 ? (
            <div className="text-center py-16 glass-panel border-dashed">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">No documents yet</h3>
              <p className="text-sm text-slate-500 mt-1">Upload your first document to share with the team.</p>
            </div>
          ) : (
              ...
              {filtered.map(r => (
                <div key={r.id} className="glass-card p-5 flex flex-col group">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {r.isPublic && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800/50">Team-Wide</span>}
                  </div>
      ...
      {tab === 'assign' && (
        <div className="max-w-2xl animate-in fade-in duration-300">
          <div className="glass-panel p-6 md:p-8 flex flex-col gap-6">
            <h3 className="text-lg font-bold text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-3">Assign Document to Specific Employees</h3>
            
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">Select Document</label>
                <select value={assignResourceId} onChange={e=>setAssignResourceId(e.target.value)} className={`${inputClass} cursor-pointer`}>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">Select Employees</label>
                <div className="flex flex-col gap-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                  {team.map(emp=>(
                    <label key={emp.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-colors ${
                      assignEmployeeIds.includes(emp.id) ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800/50' : 'bg-white border-slate-200 hover:bg-slate-50 dark:bg-slate-800/50 dark:border-slate-700 dark:hover:bg-slate-800'
                    }`}>
                      <input type="checkbox" checked={assignEmployeeIds.includes(emp.id)}
                        onChange={e=>setAssignEmployeeIds(prev=>e.target.checked?[...prev,emp.id]:prev.filter(id=>id!==emp.id))}
            ...
            <div className="pt-2">
              <button 
                ...
                disabled={assigning||!assignResourceId||assignEmployeeIds.length===0}
              ...
              >
                {assigning ? (
                  ...
                  <><CheckCircle size={18}/> Assign Document</>
                )}
              </button>
            </div>
          </div>
      ...
      {tab === 'upload' && (
        <div className="max-w-2xl animate-in fade-in duration-300">
          <div className="glass-panel p-6 md:p-8 flex flex-col gap-5">
            <h3 className="text-lg font-bold text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-3">Upload a PDF Document</h3>

            <div
              onDragOver={e=>{e.preventDefault();setDragOver(true)}} onDragLeave={()=>setDragOver(false)}
              onDrop={e=>{e.preventDefault();setDragOver(false);const f=e.dataTransfer.files[0];if(f?.type==='application/pdf')setFile(f);else toast.error('Only PDF files allowed.');}}
              ...
              <input id="pdfInput" type="file" accept=".pdf,application/pdf" className="hidden" onChange={e=>setFile(e.target.files?.[0]||null)} />
              {file ? (
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">Drag & drop PDF here or <span className="text-pmi-600 font-semibold dark:text-pmi-400">browse</span></p>
                  <p className="text-xs text-slate-400 mt-1">Max 50 MB · PDF only</p>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <label className={`flex items-center gap-3 px-3 py-2 rounded-lg border cursor-pointer h-[38px] transition-colors ${
                    ...
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-200">Visible to whole team</span>
                  </label>
                </div>
              </div>
            </div>
